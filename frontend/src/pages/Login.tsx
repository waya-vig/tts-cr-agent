        ...
        <CardContent>
          ...
          <Button
            ...
            onClick={async () => {
              try {
                setLoading(true);
                const { data } = await api.get<{ url: string; state: string }>("/auth/tiktok/authorize");
                localStorage.setItem("tiktok_oauth_state", data.state);
                window.location.href = data.url;
              } catch {
                setError("TikTok認証の開始に失敗しました");
                setLoading(false);
              }
            }}
          >
            <svg className="mr-2 h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
              <path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1v-3.5a6.37 6.37 0 00-.79-.05A6.34 6.34 0 003.15 15.2a6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.34-6.34V8.83a8.28 8.28 0 004.76 1.5V6.84a4.83 4.83 0 01-1-.15z" />
            </svg>
            TikTokでログイン
          </Button>
        </CardContent>
